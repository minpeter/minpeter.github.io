...
이 경우에는 새롭게 알아야되는 것이 있는데 X-Forwarded-For 해더이다.

해당 해더는 프록시를 지날때 유실되는 client IP를 프록시 뒤에 서비스까지 전달하기 위해 만든 사실상의 표준 해더이다.

다음 스크린샷은 whoami 서비스가 2개의 리버스 프록시 뒤에서 실행되고 있는 경우이다.

![Untitled](/images/client-ip/Untitled%201.png)

자세히보면 원본 클라이언트 IP를 넣을 수 있는 부분이 2가지 존재하는데 클라우드 플레어에서 추가한 `Cf-Connecting-Ip` 필드와 `X-Forwarded-For` 필드의 첫번재 값이다.

우선 `Cf-Connecting-Ip` 의 경우 클라우드 플레어를 사용하는 경우에만 해당되므로 제외하고 아래의 `X-Forwarded-For` 해더를 유심히 살펴보자.

첫번째 값은 분명 client ip 이다. 그럼 다음 값을 무엇일까?

바로 클라우드플레어 프록시 서버의 아이피이고 그 다음 위에선 client ip 였던 RemoteAddr의 경우 클라우드플래어 다음 리버스프록시인 traefik의 아이피이다.

아이피는 다르지만 다음과 같은 구조를 지닌다.

![Untitled](/images/client-ip/Untitled%202.png)

프록시를 하나 지날때 기존의 RemoteAddr 부분을 X-Forwarded-For 해더에 append하고 자신의 IP가 RemoteAddr에 들어가는 형식이다.
...
또한 `X-Forwarded-For` 해더는 말 그대로 HTTP 해더이다.

해더는 간단하게 조작 가능하다.

만약 `X-Forwarded-For` 해더를 처음 전송할 때 “추가” 한 상태로 전송하게 된다면 다음과 같은 형태로 전송된다.
...
위에 논리대로면 XFF 해더가 설정되어 있다. → XFF 해더 첫번째 값을 읽는다. 이기 때문에 유저가 변조하여 추가한 값이 실제 IP로 인식되게 된다.
...
| 1    | 서비스 앞에 1개의 리버스 프록시가 있으며 XFF 해더의 -1 번째 값을 사용합니다. |
| ..   | ..                                                                           |
| 3    | 서비스 앞에 3개의 리버스 프록시가 있으며 XFF 해더의 -3 번째 값을 사용합니다. |
| ..   | ..                                                                           |

이렇게 한다면 당장 사용자가 해더 값을 변조하여 보낸 경우에도 설정값을 초과하는 요청에 대해서는 그냥 무시 하기에 별 영양을 받지 않게 된다.
...
한마디로 글쓴이가 SSH 터널로 인해 우발적으로 XFF 해더에 127.0.0.1 주소가 들어가게 되었는데 이를 스택오버플로우가 “관리자”의 접근으로 인식해 권한 상승이 일어났다는 이야기이다.
...
음.. 2번에 주석의 뜻을 짐작해보면 “우린 그냥 무지성으로 XXF 해더 첫 번째 값 뺴올꺼임 ㅅㄱ ㅎ”
...
4. 신뢰할 수 있는 프록시의 IP 대역대를 설정해두고 가장 가까운 신뢰할 수 없는 XFF 해더의 IP를 사용
...
클라우드플레어를 사용한다는 가정하에 Cf-Con… 해더를 사용하거나 서버 앞단에 있는 프록시에서 X-Real-IP 값을 설정하고 그 걸 신뢰하는 방법등 다양한 방법이 있다.
...
1. XFF 헤더를 사용
2. 무조건 신뢰가 아닌 로컬주소와 클라우드플래어의 IP를 신뢰
...
그래도 WAF나 알 수 없는 원인으로 인해 XFF 해더가 강제 변경되어 고정되거나 할 수도 있기 때문에 케바케다..
